...
## Mechanism
다익스트라 알고리즘은 `Greedy`와 `Dynamic Programming`이 결합된 알고리즘입니다. 다익스트라의 기본적인 아이디어는  `"최단 거리는 최단 거리로 이루어져 있다"`는 것에 초점을 둡니다. 즉 최단 거리를 이어 붙여서 또 다른 최단 거리를 만든다는 것입니다. 이러한 철학을 바탕으로, 다익스트라는 매 단계마다 가장 가까운 노드를 찾아 그 노드의 최단 거리를 `확정`해 나가는 방식으로 동작합니다.

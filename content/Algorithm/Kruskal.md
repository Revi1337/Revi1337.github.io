...
## Kruskal
`Kruskal` 알고리즘은 무방향 가중치 그래프에서 최소 신장 트리(MST)를 찾기 위한 방법입니다. `가중치가 작은 간선부터 차례대로 선택`하며, 사이클이 생기지 않는 경우에만 간선을 추가해 트리를 `불연속적`으로 확장하는 방식으로 동작합니다.
...
> "트리를 불연속적으로 확장"의 의미는 트리가 하나의 컴포넌트가 아닌, 두개 이상의 컴포넌트로 이루어져 있을 수도 있다는 것을 의미합니다.

...
## Kruskal
`Kruskal` 알고리즘은 무방향 가중치 그래프에서 최소 신장 트리(MST)를 찾기 위한 방법입니다. `가중치가 작은 간선부터 차례대로 선택`하며, 사이클이 생기지 않는 경우에만 간선을 추가해 `불연속적`으로 트리를 확장하는 방식으로 동작합니다.
...
> "트리를 불연속적으로 확장"의 의미는 서로 연결되지 않은 여러 개의 부분 트리들이 존재할 수 있으며, 이들이 점차 하나로 합쳐지는 형태를 의미합니다.

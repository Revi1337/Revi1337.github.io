...
## 정규화
관계형 데이터베이스 설계 시, 데이터 중복과 갱신 이상을 최소화하기 위해서 `데이터를 구조화하는 작업`이다.
## 제 1 정규형 (1NF)
제 1 정규형(1NF) 은 `모든 Domain 은 원자(Atomic) 값으로 구성되어야 한다`는 내용이다. 쉽게 말해 하나의 Column 에는 두개 이상의 값이 오면 안된다는 것을 의미한다.

...
2. Java 의 해시 결과도 Index 1이지만, 이미 `Python`이 저장되어 있기 때문에 충돌이 발생합니다. 이 경우 첫 번째 시도는 `1 + (1 ^ 2) = 2`번 인덱스를 확인하고, 비어있다면 그곳에 저장합니다.
3. Kotlin 의 경우에도 마찬가지로 Index 1에서 충돌이 발생합니다. 첫 번째 시도인 Index 2에 `Java`가 저장되어 있다면, 두 번째 시도인 `1 + (2 ^ 2) = 5`번 인덱스를 확인하고 저장합니다.

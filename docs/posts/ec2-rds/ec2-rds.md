...
> 아무리 로컬이어도 외부와 통신할떄는 PublicIP 로 통신하게되는데, 이전에 RDS 에서 PublicIP 허용을 하지 않았기때문에
로컬에서 접근이 되지 않는 것이다.

![img_23.png](https://revi1337.github.io/posts/ec2-rds/img_23.png)

이렇게 해서 `EC2` 와 `RDS` 를 연동하는 법에 알아보았다. 다음 포스팅에는 
이렇게 연결된 `EC2` 와 `RDS` 에 `Spring` 을 연동하는 법을 알아보도록 하자.
